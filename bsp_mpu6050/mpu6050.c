...
MPU6050_StatusTypeDef MPU6050_UpdataTimestamp(IMU_Data_Frame *imu_data_frame)
{
  ...
  //imu_data_frame->current_timestamp = get_time_us();
  imu_data_frame->delta_t = (imu_data_frame->current_timestamp - imu_data_frame->last_timestamp) / 1000000.0;
...
static MPU6050_StatusTypeDef MPU6050_Calibrate(IMU_Data_Frame *imu_data_frame)
{
  ...
  tx_data = 0x00;
  if(I2C_Write(MPU6050_I2C_ID, MPU6050_ADDRESS, REG_PWR_MGMT_2, &tx_data, 1) != I2C_OK)
  {
    return MPU6050_I2C_WRITE_ERROR;
  }
  ...
  tx_data = 0x00;
  if(I2C_Write(MPU6050_I2C_ID, MPU6050_ADDRESS, REG_FIFO_EN, &tx_data, 1) != I2C_OK)
  {
    return MPU6050_I2C_WRITE_ERROR;
  }
  ...
  tx_data = 0x00;
  if(I2C_Write(MPU6050_I2C_ID, MPU6050_ADDRESS, REG_I2C_MST_CTRL, &tx_data, 1) != I2C_OK)
  {
    return MPU6050_I2C_WRITE_ERROR;
  }

  tx_data = 0x00;
  if(I2C_Write(MPU6050_I2C_ID, MPU6050_ADDRESS, REG_USER_CTRL, &tx_data, 1) != I2C_OK)
  {
    return MPU6050_I2C_WRITE_ERROR;
  }
  ...
  if(I2C_Read(MPU6050_I2C_ID, MPU6050_ADDRESS, REG_FIFO_COUNTH, fifo_data, 2) != I2C_OK)
  {
    return MPU6050_I2C_READ_ERROR;
  }
  uint16_t fifo_count = (fifo_data[0] << 8) | fifo_data[1];
  ...
  for(i=0; i<packet_count; i++)
  {
    ...
    if(I2C_Read(MPU6050_I2C_ID, MPU6050_ADDRESS, REG_FIFO_R_W, fifo_data, 12) != I2C_OK)
    {
      return MPU6050_I2C_READ_ERROR;
    }
    accel_temp[0] = (int16_t)(((int16_t)fifo_data[0] << 8) | fifo_data[1]);
    accel_temp[1] = (int16_t)(((int16_t)fifo_data[2] << 8) | fifo_data[3]);
    accel_temp[2] = (int16_t)(((int16_t)fifo_data[4] << 8) | fifo_data[5]);
    gyro_temp[0] = (int16_t)(((int16_t)fifo_data[6] << 8) | fifo_data[7]);
    gyro_temp[1] = (int16_t)(((int16_t)fifo_data[8] << 8) | fifo_data[9]);
    gyro_temp[2] = (int16_t)(((int16_t)fifo_data[10] << 8) | fifo_data[11]);
  ...
  if(accel_reg_bias[2] > 0)
  {
    accel_reg_bias[2] -= (int32_t)accel_sensitivity;
  }
  else
  {
    accel_reg_bias[2] += (int32_t)accel_sensitivity;
  ...
  imu_data_frame->accel_bias[0] = (float)accel_reg_bias[0] / (float)accel_sensitivity;
  imu_data_frame->accel_bias[1] = (float)accel_reg_bias[1] / (float)accel_sensitivity;
  imu_data_frame->accel_bias[2] = (float)accel_reg_bias[2] / (float)accel_sensitivity;

  imu_data_frame->gyro_bias[0] = (float)gyro_reg_bias[0] / (float)gyro_sensitivity;
  imu_data_frame->gyro_bias[1] = (float)gyro_reg_bias[1] / (float)gyro_sensitivity;
  imu_data_frame->gyro_bias[2] = (float)gyro_reg_bias[2] / (float)gyro_sensitivity;
